...
import { BookOpen, ClockCountdown, Trophy, ArrowRight, Play, SpinnerGap, Headphones } from "@phosphor-icons/react";
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-muted-foreground">{test.durationMinutes} min</span>
            </div>
            <div className="text-[10px] text-muted-foreground">{test.sectionsCount} sections · {test.questionsCount} questions</div>
          </div>
        ...
        <div className="flex items-center justify-between">
              ...
              <>
                {test.attempt.listeningBand != null && (
                  <Tooltip>
                    ...
                    <TooltipContent className="text-xs">Listening band</TooltipContent>
                  </Tooltip>
                )}
                {test.attempt.readingBand != null && (
                  <Tooltip>
                    ...
                    <TooltipContent className="text-xs">Reading band</TooltipContent>
                  </Tooltip>
                )}
              </>
            ) : (
          ...
          <Tooltip>
            ...
            <TooltipContent side="left" className="text-xs">
              {status === "not-started" ? "Start this test" : status === "in-progress" ? "Continue where you left off" : "Review your answers and score"}
            </TooltipContent>
          </Tooltip>
        </div>
      </CardContent>
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-sm border-neutral-800 bg-neutral-950 rounded-xl">
          ...
          <DialogDescription className="flex items-center gap-2 pt-1">
            ...
            {test.attempt?.band != null && (
              <span className="flex items-center gap-1 text-xs font-semibold text-white">
                <Trophy weight="bold" className="size-3 text-amber-400" /> Band {test.attempt.band}
              </span>
            )}
            {test.attempt?.listeningBand != null && (
              <Tooltip>
        ...
        {status === "completed" && (
          <p className="text-xs text-muted-foreground">This test is completed. You can review your answers but cannot retake it.</p>
        )}
        ...
        <DialogFooter>
          ...
          <Button size="sm" className="gap-1.5" disabled={timerActive && status !== "completed"} onClick={() => { onStart(test); onClose(); }}>
            ...
            {timerActive && status !== "completed" && (
              <TooltipContent side="top" className="text-xs">A test is already in progress</TooltipContent>
            )}
          </Tooltip>
        </DialogFooter>
      </DialogContent>
...
export function Tests({ onStartTest, onStopTest, timerActive }: {
  ...
  const handleStart = async (test: TestSummary) => {
    try {
      const status = getStatus(test);
      if (status === "completed" && test.attempt) {
      ...
      {activeAttempt ? (
        ...
        >
          <ExamRunner
            ...
            onListeningStart={() => onStartTest(activeAttempt.test.title, activeAttempt.test.durationMinutes * 60)}
            onExit={() => {
                ...
                <Card className="rounded-xl border-neutral-800 bg-neutral-900">
                  <CardContent className="px-3 py-2.5 flex flex-col gap-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-semibold">{tests.length ? Math.round((completed / tests.length) * 100) : 0}%</span>
                    </div>
                    <Progress value={tests.length ? (completed / tests.length) * 100 : 0} className="h-1.5 mt-1" indicatorClassName="bg-emerald-500" />
                  </CardContent>
                </Card>
              </div>
                  ...
                  {loading ? (
                    <Card className="rounded-xl border-neutral-800 bg-neutral-900">
                      <CardContent className="px-4 py-6 flex items-center gap-2 text-xs text-muted-foreground">
                        <SpinnerGap className="size-4 animate-spin" /> Loading tests...
                      </CardContent>
                    </Card>
                  ) : filtered.length === 0 ? (
